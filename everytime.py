...
class Everytime:
    ...
    def login(self, userid, password):
        ...
        body = {"userid": self.userid, "password": password, "redirect": "/"}
        response = self.session.post(url=url, data=body, headers=self.headers)
    ...
    def make_comment_dict_list(self, comment_objs):
        ...
        for comment_obj in comment_objs:
            comment = dict()
            # 부모 댓글이 존재하지 않으면 그대로, 존재하면 (대댓글)임을 표시
    ...
    def get_my_commented_article_list(self, start_num=0):
        ...
        res = self.session.post(url=url, data=body, headers=self.headers)
    ...
    def get_article_comment(self, target_article_id):  # target_id 글의 전체 내용과 댓글을 요청한다.
        ...
        article_comment_res = self.session.post(
            url=url, data=body, headers=self.headers
        )
    ...
    def get_article_list(self, target_board_id, start_num=0):
        # target_id 게시판의 글 목록을 요청한다.
        # start_num 입력시 그 번호부터 20개 요청.
        ...
        article_list_res = self.session.post(url=url, data=body, headers=self.headers)
    ...
    def async_get_article_list(self, target_board_id, start_num=0):
        # target_id 게시판의 글 목록을 요청한다.
        # start_num 입력시 그 번호부터 20개 요청.
        """target_id = target board's number or 'myarticle'(get My writing)
    ...
    # save
    def write_article(self, text, target_id, title=None, anonym=1):
        # 기본으로 익명으로 작성, anonym=0 은 아이디 공개 작성
        ...
        write_res = self.session.post(url=url, data=body, headers=self.headers)
        return write_res
...
def save_to_file(articles):
    ...
    writer.writerow(
        ["title", "text", "id", "posvote", "user_nickname", "created_at", "comments"]
    )
    for article in articles:
        comments = []
        for comment in article["comments"]:
            comments.append(list(comment.values()))
        row = list(article["article"].values()) + [
            "\n".join(
                [
                    comment[0] + "\n[" + "\t".join(comment[1:]) + "]"
...
USER_PW = "jheve-1102"
everytime = Everytime()
everytime.login(USER_ID, USER_PW)
# %%
BOARD_ID = 402030
articles = []
page_end = 2
instance = 0
while page_end:
    articles.extend(everytime.get_article_list(BOARD_ID, instance))
    instance += 20
    page_end -= 1
